    ...
    public partial class Register : Form
    {
        ...
        private void Create(String username, String password, String email, String fullname, String secretanswer)
        {
             ...
             txtSecretanswer.Text.Trim() != "")
            {
                ...
                Ping ping = new Ping();
                PingReply pingStatus = ping.Send(IPAddress.Parse("10.10.4.125")); // ping connectiın google 
                if (pingStatus.Status == IPStatus.Success)
                {
                    try
                    {
                        db.Close();
                        db.Open();
                        cmd = new SqlCommand("Insert Into user_chat(username,password,email,fullname,secretanswer,status) Values ('"
                           + username.Trim() + "','"
                           + password.Trim() + "','"
                           + email.Trim() + "','"
                           + fullname.Trim() + "','"
